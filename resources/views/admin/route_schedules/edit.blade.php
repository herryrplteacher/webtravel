                    ...
                    <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                        <div class="card-body border-b border-gray-100 dark:border-zinc-600">
                            <h6 class="mb-1 text-gray-700 text-15 dark:text-gray-100 font-semibold">
                                Form Edit Jadwal Route
                            </h6>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('update.route_schedule', $routeSchedule->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                    ...
                                    <select
                                        ...
                                        @foreach ($routes as $route)
                                            <option value="{{ $route->id }}"
                                                {{ old('route_id', $routeSchedule->route_id) == $route->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select
                                        ...
                                        <option value="daily"
                                            {{ old('schedule_type', $routeSchedule->schedule_type) == 'daily' ? 'selected' : '' }}>
                                            Setiap Hari</option>
                                        <option value="weekday"
                                            {{ old('schedule_type', $routeSchedule->schedule_type) == 'weekday' ? 'selected' : '' }}>
                                            Hari Kerja (Senin-Jumat)</option>
                                        <option value="weekend"
                                            {{ old('schedule_type', $routeSchedule->schedule_type) == 'weekend' ? 'selected' : '' }}>
                                            Akhir Pekan (Sabtu-Minggu)</option>
                                        <option value="dow"
                                            {{ old('schedule_type', $routeSchedule->schedule_type) == 'dow' ? 'selected' : '' }}>
                                            Hari Tertentu</option>
                                        <option value="date"
                                            {{ old('schedule_type', $routeSchedule->schedule_type) == 'date' ? 'selected' : '' }}>
                                            Tanggal Spesifik</option>
                                    </select>
                                    @error('schedule_type')
                                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="mb-4" id="day_of_week_field" style="display: none;">
                                    ...
                                    <select
                                        ...
                                        <option value="0"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) === 0 ? 'selected' : '' }}>
                                            Minggu</option>
                                        <option value="1"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) == 1 ? 'selected' : '' }}>
                                            Senin</option>
                                        <option value="2"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) == 2 ? 'selected' : '' }}>
                                            Selasa</option>
                                        <option value="3"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) == 3 ? 'selected' : '' }}>
                                            Rabu</option>
                                        <option value="4"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) == 4 ? 'selected' : '' }}>
                                            Kamis</option>
                                        <option value="5"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) == 5 ? 'selected' : '' }}>
                                            Jumat</option>
                                        <option value="6"
                                            {{ old('day_of_week', $routeSchedule->day_of_week) == 6 ? 'selected' : '' }}>
                                            Sabtu</option>
                                    </select>
                                    @error('day_of_week')
                                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                ...
                                <div class="mb-4" id="specific_date_field" style="display: none;">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('specific_date', $routeSchedule->specific_date?->format('Y-m-d')) }}">
                                    @error('specific_date')
                                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input
                                        ...
                                        type="time" id="depart_time" name="depart_time"
                                        value="{{ old('depart_time', $routeSchedule->depart_time ? date('H:i', strtotime($routeSchedule->depart_time)) : '') }}"
                                        required>
                                    @error('depart_time')
                                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="mb-4">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('note', $routeSchedule->note) }}"
                                ...
                                <div class="mb-6">
                                    <div class="flex items-center">
                                        <input type="checkbox" id="is_active" name="is_active" value="1"
                                            {{ old('is_active', $routeSchedule->is_active) ? 'checked' : '' }}
