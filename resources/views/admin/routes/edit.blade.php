            ...
            <div class="grid grid-cols-12 gap-6">
                <div class="col-span-12 lg:col-span-12">
                    <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                        ...
                        <div class="card-body">
                            <form action="{{ route('update.route', $route->id) }}" method="POST"
                                    ...
                                    <select
                                        ...
                                        @foreach ($services as $service)
                                            <option value="{{ $service->id }}"
                                                {{ old('service_id', $route->service_id) == $service->id ? 'selected' : '' }}>
                                                {{ $service->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input
                                        ...
                                        value="{{ old('title', $route->title) }}" placeholder="Masukkan judul route"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        ...
                                        <select
                                            ...
                                            @foreach ($locations as $location)
                                                <option value="{{ $location->id }}"
                                                    {{ old('from_location_id', $route->from_location_id) == $location->id ? 'selected' : '' }}>
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            @foreach ($locations as $location)
                                                <option value="{{ $location->id }}"
                                                    {{ old('to_location_id', $route->to_location_id) == $location->id ? 'selected' : '' }}>
                                                    {{ $location->name }} ({{ $location->getTypeLabel() }})
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            value="{{ old('price_from', $route->price_from) }}" placeholder="0"
                                            min="0">
                                        @error('price_from')
                                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            value="{{ old('duration', $route->duration) }}" placeholder="Contoh: 3 jam"
                                            maxlength="50">
                                        @error('duration')
                                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                        @enderror
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea
                                        ...
                                        id="short_desc" name="short_desc" rows="3" placeholder="Masukkan deskripsi singkat route">{{ old('short_desc', $route->short_desc) }}</textarea>
                                    @error('short_desc')
                                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    @if ($route->cover_image)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $route->cover_image) }}"
                                                alt="{{ $route->title }}" class="w-48 h-32 object-cover rounded">
                                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Current cover image
                                            </p>
                                        </div>
                                    @endif
                                    <input
                                    ...
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        Format: JPEG, JPG, PNG, WEBP. Maksimal 2MB. Biarkan kosong jika tidak ingin
                                        mengubah.
                                    </p>
                                </div>
